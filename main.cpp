...
void wineMakers()
{
    while (true)
    {
        ...
        {
            if (i != myRank)
                // IDEA: Wyślij w trakcie requesta id studenta
                sendMsg(i, REQ);
        }
...
// MOTYW DLA WYBRANIA STUDENTA
/*
W CZASIE prośby o wolne miejsca, daje id studenta któremu odpalam wino,
przy sprawdzaniu ACK sprawdzam swojego studenta z wiadomością. Jeżeli są identyczni i mam większego lamporta,
muszę wybrać nowego studenta. Jednocześnie zapisuje id winiarza aby znowu mu nie wysłać ACK.(??????????)
Dodatkowo wybierając nowego studenta od nowa startuje z wysyłaniem ACK (?????? Podchodzi pod niezły młyn w wiadomościach)
Winiarz śpi z studentem, wolny student staje się po sleepie u Winiarza, winiarz wtedy wybiera nowego losowego studenta.
